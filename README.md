...
- 무설치 릴리즈: https://github.com/ckdfuf2001/cy5_opencode_web/releases/tag/v0.3.2-cy5

## 사용 목적 
- 비개발 운영자 업무 자동화 
- Ex) 내가 네트워크 담당자이고 고객 요청 응대 자동화를 위한 커맨드들 등록/ 외부 호출 허용 
- 예시1) 고객사가 단발성으로 요청하는 특정 서버 방화벽(또는 통신이력) 현황/히스토리 요청
- 기존 : 방화벽 솔루션에서 이력 기능이 없기에 제목에 이력을 적고 등록함. (나만 이해가능/솔루션이라 api 개발 어려움) 
- 적용후: 자동으로 솔루션 접근, 리스트 추출, 내 규칙대로 리스트 정리 후 파일 저장 또는 발송 커맨드 등록 후, 고객사 에서 내 command 호출하여 데이터 수신

- Ex 내가 특정 솔루션 시스템 담당자.(서버 파일로 로그 발생)
- 고객이 특정 호출 이력 확인, 다수의 시스템 로그 파일 확인 후 응답 을 커맨드 화 하여 제공 후, 고객은 커맨드 호출 통한 이력 확인
...
- **기타 버그 fix** — 드래그드롭, 파일 붙여넣기 등 각종 호환/편의를 추가하며 버그 fix 하였습니다.
